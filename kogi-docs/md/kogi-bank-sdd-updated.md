...
| Retirement Account | Long-term personal retirement savings (SEP-IRA, 401k portability, PEP) — portable benefit | Fiat + fund units | IRA-equivalent, SEP, solo 401(k) analogues |
...
| **HSA Account** | Health Savings Account — portable benefit, pre-tax health expense fund | Fiat | Medical, dental, vision expenses; rolls over annually |
| **Portable Savings Account** | Gig platform contribution tracking account — receives % contributions from gig platforms (e.g., 4% of pre-tip earnings) | Fiat | Managed alongside benefit providers (e.g., Stride LLC) |
| **Income Protection Fund** | Reserves for income replacement during missed work, disability, or occupational accident | Fiat | Short-term disability, sick pay, income gap coverage |
| **Professional Development Account** | Portable education and training fund; receives contributions from platforms or self | Fiat | Certifications, courses, training programs |
| **Emergency Savings Account** | Rapid-access emergency buffer, separate from general savings | Fiat | Sudden expenses, income interruptions |
...
## **13.3 Portable Benefits — Overview**

The kogi-bank Portable Benefits System (KBNF-BEN) manages worker-centered benefits that remain with the individual rather than being tied to a single employer or gig platform. It is purpose-built for independent contractors, freelancers, and gig workers, giving them the same benefits infrastructure typically available only to full-time employees.

Every benefit account is a Portfolio Item in the Portfolio System — carrying full metadata, governance, lifecycle management, and audit trails. Benefit contributions from any source (platform, employer, self, collective) flow through the kogi-bank ledger and are tracked with complete attribution.

## **13.3.1 Benefit Categories**

| Category | Benefit Types | Account Type | Notes |
| :---- | :---- | :---- | :---- |
| **Health & Wellness** | Health insurance, dental, vision, HSA | HSA Account | HSA pre-tax; integrates with insurance providers via ProviderSystem |
| **Retirement & Savings** | SEP-IRA, 401k portability, Pooled Employer Plans (PEP), emergency savings | Retirement Account, Emergency Savings Account | Platform contributes % on eligible activity; worker controls allocation |
| **Paid Time Off & Income Security** | Paid sick days, vacation time, income replacement for missed work | Income Protection Fund | Contribution accumulates as a reserve; disbursed on claim via Oba |
| **Insurance & Protection** | Occupational accident insurance, disability, workers' compensation | Income Protection Fund | Integrated with occupational insurance providers |
| **Professional Development** | Education accounts, training fund, certification credits | Professional Development Account | Employer or platform contributions + self-directed |
| **Platform Contributions** | Gig platform % contributions (e.g. 4% of pre-tip earnings) | Portable Savings Account | Tracking partner platform contributions via provider API; custodied by benefit manager (e.g. Stride LLC) |

## **13.3.2 Portable Benefits Data Model**

```
BenefitsAccount (extends Account)
├── benefit_type: health | dental | vision | hsa | retirement_sep | retirement_401k |
│                 retirement_pep | pto | income_replacement | disability | workers_comp |
│                 professional_dev | emergency_savings | portable_savings
├── provider_id → linked ProviderSystem provider (insurance co., retirement platform, HSA custodian)
├── contribution_sources[]
│   ├── { source_id, source_type: self | platform | gig_platform | employer | collective }
│   ├── { amount | pct_of_earnings, frequency: per_payment | monthly | annual }
│   └── { platform_name, contribution_rule_id }
├── balance | coverage_amount | coverage_details
├── vesting_schedule: { cliff_months, vesting_months, vested_pct }
├── eligibility_rules: PolicySet
├── claim_workflow_id → ApprovalWorkflow for disbursement
├── tax_treatment: pre_tax | post_tax | tax_free
└── linked_account_id → parent Account (kogi-bank)
```

## **13.3.3 Platform Contribution Tracking**

When a gig platform (e.g., DoorDash, Uber, Instacart) contributes a percentage of earnings to a worker's portable savings account, kogi-bank tracks these contributions via the ProviderSystem integration:
...
## **13.3.4 Benefits Engine Integration**

The kogi-engine BenefitsEngine provides:
...
- Contribution forecasting — projects benefit account balances given current income trajectory and contribution rules
...
## **13.4 Grants & Microfinancing — Overview**

kogi-bank provides a complete grants and microfinancing subsystem for independent workers, collectives, cooperatives, and autonomous organizations. It handles grant discovery, application, fund disbursement, microloan origination, repayment tracking, and impact reporting — all governed by the Portfolio System and tracked in the double-entry ledger.

## **13.4.1 Grant System**

| Component | Description |
| :---- | :---- |
| **Grant Registry** | Catalog of available grants from government, foundation, platform, and community sources; searchable by eligibility, category, amount, and deadline |
| **Grant Application** | Structured application workflow linked to a Portfolio Program or Project; supporting documents attached as Portfolio Artifacts |
| **Grant Fund Disbursement** | Awarded grant funds deposited to a dedicated Grant Account; disbursed against milestones per grant agreement |
| **Grant Reporting** | Impact reporting and milestone reporting artifacts auto-generated from Portfolio System data; submitted to grant provider |
| **Grant Impact Ledger** | Tracks how grant funds are deployed across portfolio projects; feeds analytics for future grant eligibility |
| **Community Grant Pools** | Organizations and collectives can create shared grant pools funded by member contributions; governed by governance proposals |

**Grant Data Model:**
```
Grant (PortfolioItem — kind: Grant)
├── grant_id · grant_name · grant_type: government | foundation | platform | community
├── grantor_id → external Provider or internal Organization entity
├── recipient_id → User | Organization | Collective | Cooperative
├── amount · currency · award_date · expiry_date
├── application_id → GrantApplication (linked Artifact)
├── disbursement_schedule[] → { milestone_id, amount, condition, status }
├── grant_account_id → dedicated Grant Account in kogi-bank
├── reporting_requirements[] → { report_type, due_date, status }
├── impact_metrics[] → { metric_name, baseline, target, actual }
└── portfolio_component_id → full Portfolio Item metadata
```

## **13.4.2 Microfinancing System**

| Component | Description |
| :---- | :---- |
| **Microloan Origination** | Workers and organizations can apply for microloans from community lenders, platform funds, or cooperative pools; applications linked to portfolio context |
| **Loan Terms** | Configurable principal, interest rate, repayment schedule, and collateral (portfolio assets as collateral) |
| **Repayment Tracking** | Automated repayment reminders, ledger entries for each payment; delinquency alerts via kogi-engine RiskEngine |
| **Community Lending Pools** | Collectives and cooperatives can establish shared lending pools where members both contribute capital and access loans at preferential rates |
| **Credit Scoring Integration** | kogi-engine RiskEngine computes a platform credit score from portfolio activity, income history, repayment history, and community standing |
| **Crowdlending** | Multiple lenders can contribute fractional amounts to a single microloan; pro-rated repayments distributed automatically |
...
## **13.5 Group Economics — Overview**

Group Economics encompasses all collective financial structures and instruments on the kogi platform — from cooperative treasury management and shared revenue distribution to group investment pools and collective resource acquisition. Every group economics instrument is backed by the kogi-bank account and ledger infrastructure, with governance enforced through kogi-organizations.

## **13.5.1 Group Economics Instruments**

| Instrument | Description | Entity Types | Governance |
| :---- | :---- | :---- | :---- |
| **Cooperative Treasury** | Multi-sig controlled shared fund for a cooperative's operations, payroll, and investment | Cooperative | Multi-sig + governance vote for large disbursements |
| **Revenue Sharing Pool** | Collective revenue is aggregated and distributed among members according to a defined formula (hours, equity, contribution) | Cooperative, Collective | Automated computation via BenefitsEngine + manual approval |
| **Group Investment Pool** | Members collectively deploy capital into portfolio assets, equity campaigns, or external investments | Org, Collective, Cooperative | Governance proposal required; investment decision tracked |
| **Mutual Aid Fund** | Peer-support pool for member emergencies; funded by member contributions; governed by mutual aid policy | Collective, Cooperative | Claims reviewed by stewards or elected committee |
| **Resource Pooling Account** | Shared account for collectively purchasing and managing shared resources (tools, software, compute) | Any group | Majority approval for purchases above threshold |
| **Crowdresourcing Pool** | Community-sourced contributions of labor, assets, and knowledge aggregated and attributed per contributor | Any community | Contribution ledger with provenance tracking |
| **Collective Escrow** | Multi-party escrow for group deals, shared purchases, or collaborative project funding | Any group | M-of-N multi-sig release |

## **13.5.2 Revenue Distribution Engine**

When a cooperative or collective distributes revenue or profit among members, kogi-bank executes a distribution event:

1. Treasurer or Oba triggers distribution computation
2. BenefitsEngine computes each member's distribution share based on configured formula:
   - Hours contributed (time-tracked via kogi-office)
   - Equity share percentage (from Equity Pool account)
   - Patronage / contribution score (from PortfolioSystem analytics)
   - Governance bonus pool allocation (from governance vote)
...
7. Oba notifies each member: "Q1 distribution of $3,189.40 has been deposited."

## **13.5.3 Equity Crowdfunding**

kogi-bank provides a complete equity crowdfunding infrastructure for organizations and independent workers raising community capital:

| Component | Description |
| :---- | :---- |
| **Campaign Types** | Equity (Reg CF, Reg A+), Revenue Share, SAFE Note, Convertible Note, Donation, Reward-based |
| **Cap Table Management** | Investor equity positions tracked in kogi-bank Equity Accounts; vesting schedules enforced; dilution computed on new rounds |
| **Escrow Management** | Campaign funds held in dedicated Campaign Account / Escrow Reserve until funding goal met; auto-disbursed on success; auto-refunded on failure |
| **Investor Portal** | Investor-facing view of their equity positions, expected returns, distribution history, and portfolio performance |
| **Revenue Share Engine** | For revenue-share instruments: automated periodic computation and distribution of investor returns from operating revenue |
| **Regulatory Compliance** | Reg CF campaign limits enforced; accredited investor verification hooks via ProviderSystem (e.g., Parallel Markets, Carta) |
| **Secondary Market** | Future: peer-to-peer transfer of equity positions with governance approval |
