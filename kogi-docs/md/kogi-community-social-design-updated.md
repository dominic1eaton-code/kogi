...
| Microloan | A small-denomination loan originated within the community or platform; tracked in kogi-bank |
| BenefitsEngine | Engine subsystem for benefits eligibility scoring, contribution tracking, and coverage optimization |
| CollaborationEngine | Engine subsystem coordinating shared portfolio workflows, contribution attribution, and conflict resolution |
...
The Kogi Community layer is where shared portfolios and portfolio collaboration come to life. Any space, organization, collective, cooperative, or federation can create and manage a shared portfolio — a portfolio jointly owned and governed by multiple members, with full attribution tracking, governance controls, and economic instruments for distributing value.

## **15.2 Shared Portfolio Types**
...
| **Cooperative Portfolio** | Worker-owned portfolio with equity distribution | Full governance; all major changes require member vote |
...
## **15.3 Portfolio Resource Sharing**

Within a shared portfolio or community space, members can share the following resource types:
...
## **15.4 Contribution Attribution**

All contributions to a shared portfolio item are tracked with full attribution:

```
ContributionRecord
├── contributor_id → user | organization
├── contribution_type: labor | capital | asset | knowledge | artifact | code | design | data
├── portfolio_component_id → item contributed to
├── contribution_value: { amount, unit, valuation_method }
├── timestamp · version_id
├── attribution_weight → used in revenue/equity distribution calculations
├── governance_status: pending_review | accepted | rejected | merged
└── linked_ledger_entry_id → kogi-bank ledger entry for capital contributions
```

## **15.5 Portfolio Collaboration Workflow**

```
INVITE / OPEN CONTRIBUTION
        ↓
CONTRIBUTION SUBMITTED (labor, asset, capital, knowledge)
        ↓
STEWARD / GOVERNANCE REVIEW (configurable: auto-accept | review | vote)
        ↓
MERGE / ACCEPT → ContributionRecord created → Ledger updated
        ↓
ATTRIBUTION WEIGHT UPDATED → feeds distribution engine
        ↓
DISTRIBUTION EVENT (periodic or milestone-triggered)
        ↓
kogi-bank executes distribution → member accounts credited
```
...
## **16.1 Booking & Scheduling System (KBKG)**

The Kogi Community and Office layers provide a centralized booking and scheduling infrastructure for independent workers managing multiple clients, venues, projects, and collaborators.

| Feature | Description |
| :---- | :---- |
| **Centralized Calendars** | Unified calendar aggregating all bookings, events, project milestones, and personal schedules across kogi-office and kogi-community |
| **Multi-Worker / Multi-Artist Dashboards** | Dashboard view for organizations and collectives showing all member schedules, availability windows, and booking conflicts |
| **Conflict Detection** | Automated detection of scheduling conflicts across team members, resources, and venues; Oba alerts on conflicts before confirmation |
| **Event Booking & Reservation Management** | Full booking lifecycle: request → quote → confirm → deposit → delivery → close; integrated with kogi-bank for deposit handling |
| **Availability Management** | Worker-defined availability windows with blackout dates, recurring blocks, and buffer time between bookings |
| **Resource Booking** | Book equipment, studio space, venues, or shared resources alongside worker time; linked to logistics tracking |
| **Booking Analytics** | Utilization rates, booking-to-revenue conversion, peak demand periods, cancellation rates |

**Booking Data Model:**
```
Booking (PortfolioItem — kind: Booking)
├── booking_id · title · type: event | gig | session | venue | equipment | custom
├── client_id → CRM Contact or User
├── worker_ids[] → assigned workers/artists
├── resource_ids[] → booked equipment/venues
├── schedule: { start_time, end_time, timezone, recurrence_rule }
├── status: inquiry | quoted | confirmed | deposited | in_progress | completed | cancelled
├── financial: { quote_amount, deposit_amount, balance_due, invoice_id }
├── logistics_plan_id → linked LogisticsPlan
└── portfolio_component_id → full metadata
```

## **16.2 CRM & Lead Management System (KCRM)**

| Feature | Description |
| :---- | :---- |
| **Lead Capture Forms** | Embeddable forms linked to worker or organization profile pages; leads flow directly into CRM pipeline |
| **Client Database Management** | Searchable, filterable client records with contact info, booking history, communication log, tags, and notes |
| **Pipeline Management** | Kanban-style pipeline for moving leads through stages: inquiry → qualified → proposal → negotiation → booked → retained |
| **Automated Follow-up Emails** | Triggered email sequences on lead stage changes, quote expiry, post-event follow-ups; managed via kogi-providers (Gmail, Mailchimp) |
| **Lead Scoring** | kogi-engine CRMEngine scores leads by probability-to-book based on interaction history, budget signals, and match quality |
| **Client Communication Log** | All messages, emails, and calls with a client automatically logged against their CRM record |
| **Retention Analytics** | Repeat booking rates, lifetime value, churn signals; Oba recommends re-engagement campaigns |

## **16.3 Contracts & Invoicing**
...
| **E-Signatures** | Integrated e-signature workflow (DocuSign, HelloSign via ProviderSystem); status tracked in contract record |
| **Automated Payment Reminders** | Configurable reminder sequences for deposit due, balance due, and overdue payments; sent via kogi-bank invoice system |
...
## **16.4 Logistics & Communication System (KLGS)**
...
| **Resource Allocation** | Assign equipment, vehicles, and crew to specific dates/times/locations; conflict detection prevents double-allocation |
| **In-App Communication Tools** | Dedicated logistics room per booking/tour; broadcast messages to all assigned crew; real-time status updates |
| **Logistics Tracking** | Live tracking of: equipment rental status, transportation arrangements, crew check-ins, venue setup progress |
| **Equipment Rental Management** | Inventory of owned and rented equipment; rental period tracking; return reminders; cost ledgering in kogi-bank |
| **Transportation Management** | Vehicle assignments, routing, mileage tracking, and driver schedules |
| **Crew Schedule Management** | Role-based crew assignments per event/day; call times, breaks, and wrap times; automated schedule distribution |
| **Finance & Reporting** | Real-time expense tracking against event budget; P&L per booking; tax report generation for event-related expenses |

**LogisticsPlan Data Model:**
```
LogisticsPlan (PortfolioItem — kind: LogisticsPlan)
├── plan_id · booking_id · event_name
├── itinerary_stops[] → { location, arrival_time, departure_time, venue_id, notes }
├── equipment_assignments[] → { equipment_id, rental_or_owned, pickup_time, return_time }
├── transportation_assignments[] → { vehicle_id, driver_id, route, departure_time }
├── crew_assignments[] → { worker_id, role, call_time, wrap_time, location }
├── budget: { total_budget, spent, remaining } → linked kogi-bank Campaign Account
├── communication_room_id → KRMS logistics room
└── status: planning | confirmed | in_progress | wrapped | archived
```
...
## **17.1 Initiatives Overview**

Initiatives are platform-coordinated programs for collective action within the Kogi Community. They provide a structured framework for organizations, collectives, cooperatives, and community spaces to coordinate around shared goals — beyond individual projects and governance proposals.

| Initiative Type | Description |
| :---- | :---- |
| **Portable Benefits Initiative** | Coordinate a group of independent workers to pool buying power for health, dental, vision, or retirement products through a Pooled Employer Plan (PEP) or collective insurance arrangement |
| **Grants Initiative** | Community-organized grant discovery, collective application, and fund distribution program; managed by a designated steward committee |
...
| **Group Economics Initiative** | Formal program for collective purchasing, shared revenue arrangements, cooperative investment, or group crowdfunding |
...
## **17.2 Initiative Data Model**

```
Initiative (PortfolioItem — kind: Initiative)
├── initiative_id · name · type (see types above)
├── space_id → host Space
├── organization_id → sponsoring Organization (optional)
├── stewards[] → designated initiative managers
├── participants[] → enrolled members
├── linked_accounts[] → kogi-bank accounts (grant pool, lending pool, etc.)
├── linked_proposals[] → governance proposals governing the initiative
├── linked_programs[] → portfolio programs associated with the initiative
├── milestones[] · status: draft | active | completed | archived
├── impact_metrics[] → tracked outcomes
└── portfolio_component_id → full metadata
```
