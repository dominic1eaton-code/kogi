...
//! Uses hypergrid::crdt::VectorClock and CrdtLog for causal ordering.
...
use crate::crdt::PortfolioCrdtLog;
...
impl PortfolioFederation {
    ...
    pub fn mark_seen(&mut self, id: PeerId) {
        if let Some(p) = self.peers.get_mut(&id) { p.last_seen = Utc::now(); }
    }
    ...
    pub fn delta_for_peer<'a>(
        &self, peer_id: &str, log: &'a PortfolioCrdtLog,
    ) -> &'a [crate::crdt::CrdtOperation] {
        log.delta_since(peer_id)
    }
}
