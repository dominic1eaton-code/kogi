...
pub fn TimePointToDateTime(tp: TimePoint) DateTime {
    ...
    const day: u5 = @as(u5, remaining / 86400) + 1;
...
pub const Clock = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Clock {
        ...
        return Clock{
            .allocator = allocator,
            .base_time = @intCast(current_ts),
            .start_time = @intCast(current_ts),
    ...
    pub fn pause(self: *Clock) void {
        ...
        if (!self.paused) {
            const current_ts = std.time.nanoTimestamp();
            self.pause_offset = current_ts - self.base_time;
            self.paused = true;
        }
    }
    pub fn unpause(self: *Clock) void {
        ...
        if (self.paused) {
            const current_ts = std.time.nanoTimestamp();
            self.base_time = current_ts - self.pause_offset;
            self.paused = false;
        }
    }
...
pub const Timer = struct {
    ...
    pub fn startTimer(allocator: std.mem.Allocator, label: []const u8) !Timer {
        return Timer{
    ...
    pub fn elapsedDuration(self: Timer) Duration {
        ...
        return self.start_time.duration(end);
    }

    pub fn stop(self: Timer) void {
        const dur = self.elapsedDuration();
...
pub const StopWatch = struct {
    ...
    pub fn pause(self: *StopWatch) void {
        if (self.is_running) {
            const timestamp = TimePoint.now();
            const elapsed_duration = self.start_time.duration(timestamp);
            self.total_time.nanos += elapsed_duration.nanos;
            self.is_running = false;
        }
    }

    pub fn resumeWatch(self: *StopWatch) void {
        if (!self.is_running) {
    ...
    pub fn elapsed(self: StopWatch) Duration {
        ...
        if (self.is_running) {
            const now_ts = TimePoint.now();
            const current_duration = self.start_time.duration(now_ts);
            result.nanos += current_duration.nanos;
        }
        return result;
    }
