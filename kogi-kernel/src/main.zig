...
pub fn main() !void {
    ...
    std.debug.print(
        "kogi-kernel mvp: modules={d} isolated={d} events={d} mem={d}/{d} mode={s}\n",
        .{ s.module_count, s.isolated_module_count, s.event_count, s.memory_used_bytes, s.memory_total_bytes, @tagName(s.mode) },
    );
}
