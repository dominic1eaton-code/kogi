// =============================================================================
//  portfolio_system.rs — Kogi OS · Portfolio System  (Reconciled v2.1)
...
//
//  v2.1 — Tool Management System integration
//    · ComponentData now carries `toolbox_ids: Vec<ToolBoxId>` so that any
//      portfolio element (item or container) can own one or more ToolBoxes.
//    · PortfolioSystem exposes `attach_toolbox` / `detach_toolbox` helpers.
//    · See tool_management_system.rs for the full ToolBox/ToolSet/ToolKit/
//      ToolChain/ToolOrchestration implementation.
//
//  ─────────────────────────────────────────────────────────────────────────────
...
//      │     │                       toolbox_ids  ← NEW (TMS integration)
...
// ── Tool Management System cross-reference ────────────────────────────────────
/// Opaque reference to a ToolBox in the ToolManagementSystem.
/// Resolved at runtime via ToolManagementSystem::get_toolbox(id).
/// See tool_management_system.rs for the full ToolBox definition.
...
pub struct ComponentData {
    ...
    /// IDs of ToolBoxes (from ToolManagementSystem) attached to this element.
    /// A ToolBox contains ToolSets → ToolKits / ToolChains / ungrouped tools.
    /// Use PortfolioSystem::attach_toolbox / detach_toolbox to manage.
...
// =============================================================================
// §29-TMS — TOOL MANAGEMENT SYSTEM INTEGRATION
// =============================================================================
//
//  These methods integrate the PortfolioSystem with the ToolManagementSystem.
//  Every portfolio Component (Item or Container) can own one or more ToolBoxes.
//  A ToolBox contains ToolSets, which are composed of ToolKits, ToolChains, and
//  ungrouped tools.  The ToolBox itself is resolved at runtime through a
//  ToolManagementSystem instance — the portfolio stores only the ToolBoxId.
//
//  Typical usage:
//
//    1. Create a ToolBox in ToolManagementSystem:
//         let box_id = tms.create_toolbox(ToolBox::new("Dev Toolbox", "alice"));
//
//    2. Attach it to a portfolio component:
//         portfolio_system.attach_toolbox(user_id, &component_id, box_id)?;
//
//    3. Look up attached boxes:
//         let ids = portfolio_system.toolbox_ids(&component_id)?;
//         for id in ids { let tb = tms.get_toolbox(&id).unwrap(); }

impl PortfolioSystem {
    /// Attach a ToolBox (by ID) to a portfolio component.
    /// The `toolbox_id` must refer to a valid ToolBox in the
    /// ToolManagementSystem; this method only records the reference here.
    pub fn attach_toolbox(
        ...
        let component = self.read_mut(component_id)?;
        if !component.data().users.can(&actor, &ActionKind::Edit) {
            return Err(PortfolioError::PermissionDenied {
                user_id: actor,
                action: "attach_toolbox".to_string(),
            });
        }
        let data = component.data_mut();
        if !data.toolbox_ids.contains(&toolbox_id) {
            data.toolbox_ids.push(toolbox_id);
            data.metadata.touch(&self.node_id.clone());
        }
        Ok(())
    }

    /// Detach a ToolBox from a portfolio component.
    pub fn detach_toolbox(
        &mut self,
        actor: UserId,
        component_id: &ComponentId,
        toolbox_id: &ToolBoxId,
        ...
        if !component.data().users.can(&actor, &ActionKind::Edit) {
            return Err(PortfolioError::PermissionDenied {
                user_id: actor,
                action: "detach_toolbox".to_string(),
            });
        }
        let data = component.data_mut();
        data.toolbox_ids.retain(|id| id != toolbox_id);
        data.metadata.touch(&node);
        Ok(())
    }

    /// Return all ToolBox IDs attached to a portfolio component.
    pub fn toolbox_ids(&self, component_id: &ComponentId) -> PortfolioResult<&[ToolBoxId]> {
        Ok(&self.read(component_id)?.data().toolbox_ids)
    }

    /// Return `true` if the given ToolBox is attached to the component.
    pub fn has_toolbox(&self, component_id: &ComponentId, toolbox_id: &ToolBoxId) -> PortfolioResult<bool> {
        Ok(self.read(component_id)?.data().toolbox_ids.contains(toolbox_id))
    }

    /// Collect every ToolBoxId across all components in this PortfolioSystem.
    pub fn all_toolbox_ids(&self) -> Vec<ToolBoxId> {
        self.components
            .values()
...
mod tests {
    ...
    // ── §29-TMS: Tool Management System integration tests ────────────────────

    #[test]
    fn test_attach_toolbox_to_component() {
        let mut sys = PortfolioSystem::new("node-1");
        let user = owner();

        let item_id = sys.create_item(user, "My Project", ItemType::Project(ProjectPayload {
            project_type: ProjectType::Software,
            methodology: None, start_date: None, end_date: None,
            sprints: vec![], backlog: vec![], releases: vec![],
        })).unwrap();

        let toolbox_id = Uuid::new_v4(); // Would be created via ToolManagementSystem
        sys.attach_toolbox(user, &item_id, toolbox_id).unwrap();

        assert!(sys.has_toolbox(&item_id, &toolbox_id).unwrap());
        assert_eq!(sys.toolbox_ids(&item_id).unwrap().len(), 1);
    }

    #[test]
    fn test_detach_toolbox_from_component() {
        let mut sys = PortfolioSystem::new("node-1");
        let user = owner();

        let item_id = sys.create_item(user, "My Asset", ItemType::Asset(AssetPayload {
            asset_type: AssetType::Digital, valuation: None, currency: None, acquired_at: None,
        })).unwrap();

        let tb1 = Uuid::new_v4();
        let tb2 = Uuid::new_v4();
        sys.attach_toolbox(user, &item_id, tb1).unwrap();
        sys.attach_toolbox(user, &item_id, tb2).unwrap();
        assert_eq!(sys.toolbox_ids(&item_id).unwrap().len(), 2);

        sys.detach_toolbox(user, &item_id, &tb1).unwrap();
        assert_eq!(sys.toolbox_ids(&item_id).unwrap().len(), 1);
        assert!(!sys.has_toolbox(&item_id, &tb1).unwrap());
        assert!(sys.has_toolbox(&item_id, &tb2).unwrap());
    }

    #[test]
    fn test_idempotent_toolbox_attach() {
        let mut sys = PortfolioSystem::new("node-1");
        let user = owner();

        let item_id = sys.create_item(user, "Artifact", ItemType::Artifact(ArtifactPayload {
            artifact_type: "doc".into(), file_refs: vec![], produced_by: vec![],
        })).unwrap();

        let tb = Uuid::new_v4();
        sys.attach_toolbox(user, &item_id, tb).unwrap();
        sys.attach_toolbox(user, &item_id, tb).unwrap(); // duplicate — must not double-register
        assert_eq!(sys.toolbox_ids(&item_id).unwrap().len(), 1);
    }

    #[test]
    fn test_all_toolbox_ids_across_system() {
        let mut sys = PortfolioSystem::new("node-1");
        let user = owner();

        let id1 = sys.create_item(user, "Item A", ItemType::Artifact(ArtifactPayload {
            artifact_type: "doc".into(), file_refs: vec![], produced_by: vec![],
        })).unwrap();
        let id2 = sys.create_item(user, "Item B", ItemType::Asset(AssetPayload {
            asset_type: AssetType::Digital, valuation: None, currency: None, acquired_at: None,
        })).unwrap();

        let tb1 = Uuid::new_v4();
        let tb2 = Uuid::new_v4();
        sys.attach_toolbox(user, &id1, tb1).unwrap();
        sys.attach_toolbox(user, &id2, tb2).unwrap();

        let all = sys.all_toolbox_ids();
        assert!(all.contains(&tb1));
        assert!(all.contains(&tb2));
        assert_eq!(all.len(), 2);
    }

    #[test]
    fn test_toolbox_permission_denied() {
        let mut sys = PortfolioSystem::new("node-1");
        let owner_id = owner();
        let viewer   = owner();

        let item_id = sys.create_item(owner_id, "Restricted Item", ItemType::Artifact(ArtifactPayload {
            artifact_type: "contract".into(), file_refs: vec![], produced_by: vec![],
        })).unwrap();

        // viewer has no permissions — attach should be denied
        let tb = Uuid::new_v4();
        let result = sys.attach_toolbox(viewer, &item_id, tb);
        assert!(result.is_err());
        match result.unwrap_err() {
            PortfolioError::PermissionDenied { .. } => {}
